 ...
 * {@link SingletonWithStaticFactoryMethodDoubleCheckLocking#getInstance()}
 ...
 * <p>
 * Issue: Synchronized block is called only one time. But without volatile modifier, it’s possible
 * for another thread in Java to see half initialized state of
 * {@link SingletonWithStaticFactoryMethodDoubleCheckLocking#singleton} variable. This issue is
 * solved in the {@link SingletonWithStaticFactoryMethodDoubleCheckLockingVolatile}.
 * <p>
 * This approach is described in Joshua Bloch's book "Effective Java" (p.332, 334).
 */
public class SingletonWithStaticFactoryMethodDoubleCheckLocking {

    private static SingletonWithStaticFactoryMethodDoubleCheckLocking singleton;

    private SingletonWithStaticFactoryMethodDoubleCheckLocking() {
        if (singleton != null) {
    ...
    public static SingletonWithStaticFactoryMethodDoubleCheckLocking getInstance() {
        if (singleton == null) {

            synchronized (SingletonWithStaticFactoryMethodDoubleCheckLocking.class) {

                if (singleton == null) {
                    singleton = new SingletonWithStaticFactoryMethodDoubleCheckLocking();
                }
            }
        }
